来自：[Swift中的String API](https://www.jianshu.com/p/4b5952dfb2c5)
...
如果你想要处理 UTF-8 或者 Unicode 码位，**你需要将原始字符串转化成 UTF-8 或者 UTF-32 表示然后再对结果进行操作。这种处理方式更多是将字符串视为数据对象，而不是视为字符串本身，所以在转换时并不是很方便。** 现在我们想通过字符簇的方式遍历:
